...
describe('DbSaveSurveyResult UseCase', () => {
  ...
  test('Should call LoadResultRepository with correct values', async () => {
    const { sut, loadSurveyResultRepositorySpy } = makeSut()
    const surveyResultData = mockSaveSurveyResultParams()
    await sut.save(surveyResultData)
    expect(surveyResultData.surveyId).toBe(loadSurveyResultRepositorySpy.surveyId)
  })

  test('Should call SaveResultRepository with correctvalues', async () => {
    const { sut, saveSurveyResultRepositorySpy } = makeSut()
    const surveyResultData = mockSaveSurveyResultParams()
    await sut.save(surveyResultData)
    expect(surveyResultData).toEqual(saveSurveyResultRepositorySpy.saveSurveyResultParams)
  })
